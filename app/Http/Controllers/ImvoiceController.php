...
class ImvoiceController extends Controller
{
    ...
    public function cetak_pdf(order $order)
    {   

        $order=Order::all();
        $profile = Profile::first();
        $pdf = PDF::loadview('order.receipt', ['profile' => $profile], ['order' => $order]);
        return $pdf->download('laporan-mahasiswa-pdf.pdf');
    }
    public function receipt(Order $order)
    {
        $profile = Profile::first();
       // $pdf = PDF::loadview('order.receipt', compact('order','profile'))
